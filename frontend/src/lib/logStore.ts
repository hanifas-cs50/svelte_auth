...
// const MS3_URL = `${env.PUBLIC_BACKEND_URL}-5003.app.github.dev/ms3`;
const MS3_URL = `${env.PUBLIC_BACKEND_URL}:5003/ms3`;
...
export async function initializeLogs() {
	try {
		const res2 = await fetch(`${MS3_URL}/logs`);
...
export async function getLog(id: number) {
	try {
		const res = await fetch(`${MS3_URL}/log/${id}`);
